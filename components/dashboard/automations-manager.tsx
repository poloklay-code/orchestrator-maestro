...
import { Plus, Search, Zap, Pencil, Trash2, Play, Pause } from "lucide-react"

const AUTOMATION_PLATFORMS = [
  "n8n", "Make (Integromat)", "ManyChat", "Zapier", "ActiveCampaign",
  "RD Station", "HubSpot", "Kommo (amoCRM)", "Typebot", "BotConversa",
  "WhatsApp Business API", "Twilio", "Evolution API", "Chatwoot",
]
...
export function AutomationsManager({ initialAutomations, clients }: { initialAutomations: Automation[]; clients: Array<{ id: string; name: string }> }) {
  const [automations, setAutomations] = useState(initialAutomations)
  const [search, setSearch] = useState("")
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ client_id: "", platform: "", workflow_name: "", workflow_id: "", status: "active", trigger_type: "webhook" })
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from("automations").insert({ ...form, client_id: form.client_id || null })
    if (error) { toast.error("Erro ao cadastrar"); return }
    await supabase.from("audit_logs").insert({ action: `Nova automacao: ${form.workflow_name || form.platform}`, entity_type: "automation", source: "admin", severity: "info" })
    toast.success("Automacao cadastrada")
    setShowForm(false)
    const { data } = await supabase.from("automations").select("*, clients(name)").order("created_at", { ascending: false })
  ...
  const toggleStatus = async (auto: Automation) => {
    ...
    await supabase.from("audit_logs").insert({ action: `Automacao ${newStatus === "active" ? "ativada" : "pausada"}: ${auto.workflow_name || auto.platform}`, entity_type: "automation", source: "admin", severity: "info" })
    toast.success(`Automacao ${newStatus === "active" ? "ativada" : "pausada"}`)
    setAutomations(automations.map((a) => a.id === auto.id ? { ...a, status: newStatus } : a))
  }

  const handleDelete = async (auto: Automation) => {
    if (!confirm(`Excluir automacao ${auto.workflow_name || auto.platform}?`)) return
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90">
          <Plus className="w-4 h-4" /> Nova Automacao
        </button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-lg space-y-4">
            <h2 className="text-lg font-semibold text-foreground">Nova Automacao</h2>
            <div className="grid gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.workflow_name} onChange={(e) => setForm({ ...form, workflow_name: e.target.value })}
                    className="w-full h-10 bg-secondary border border-border rounded-lg px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div>
      ...
      <div className="grid gap-3">
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">Nenhuma automacao encontrada</p>
          </div>
        ) : (
          filtered.map((auto) => (
            <div key={auto.id} className="flex items-center gap-4 p-4 rounded-xl border border-border bg-card hover:border-primary/20 transition-all group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
                  ...
                  <span>{auto.clients?.name || "Geral"}</span>
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => handleDelete(auto)} className="p-2 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive">
