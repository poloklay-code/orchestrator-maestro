...
import { Brain, Star, Plus, Search, Lightbulb, Trophy, AlertTriangle, Trash2, Tag } from "lucide-react"
...
const CATEGORY_CONFIG = {
  golden: { label: "Decisao de Ouro", color: "text-yellow-400 bg-yellow-500/10", icon: Trophy },
  pattern: { label: "Padrao Identificado", color: "text-primary bg-primary/10", icon: Brain },
  risk: { label: "Risco Mapeado", color: "text-red-400 bg-red-500/10", icon: AlertTriangle },
  insight: { label: "Insight Estrategico", color: "text-blue-400 bg-blue-500/10", icon: Lightbulb },
}

export function StrategicMemory() {
  ...
  const handleSave = async () => {
    ...
    await supabase.from("audit_logs").insert({ action: `Memoria estrategica: ${form.title}`, entity_type: "memory", source: "admin", severity: "info", details: { category: form.category } })
    setShowForm(false)
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 space-y-3">
          ...
          <textarea placeholder="Padrao abstrato identificado (sem dados sensiveis) *" value={form.pattern} onChange={(e) => setForm({ ...form, pattern: e.target.value })} rows={2}
              ...
              <div key={dec.id} className="p-4 rounded-xl border border-border bg-card hover:border-primary/20 transition-all group">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className={`flex items-center gap-1 text-[10px] font-mono px-2 py-0.5 rounded-full ${cfg.color}`}>
                ...
                {dec.outcome && <p className="text-xs text-muted-foreground leading-relaxed"><strong className="text-green-400">Resultado:</strong> {dec.outcome}</p>}
                {dec.tags.length > 0 && (
                  <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                    <Tag className="w-3 h-3 text-muted-foreground" />
                    {dec.tags.map((t) => <span key={t} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-secondary text-muted-foreground">{t}</span>)}
                  </div>
                )}
              </div>
            )
