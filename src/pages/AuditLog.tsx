...
interface Log {
  id: string; action: string; entity_type: string | null; severity: string; source: string; created_at: string;
}

const demoLogs: Log[] = [
  { id: "1", action: "Sistema online há 24h sem interrupções", entity_type: "system", severity: "info", source: "admin", created_at: new Date().toISOString() },
  { id: "2", action: "Nova automação concluída com sucesso", entity_type: "automation", severity: "info", source: "admin", created_at: new Date(Date.now() - 3600000).toISOString() },
  { id: "3", action: "Tentativa de acesso não autorizado bloqueada", entity_type: "security", severity: "error", source: "system", created_at: new Date(Date.now() - 7200000).toISOString() },
  { id: "4", action: "Cliente atualizado: João Silva", entity_type: "client", severity: "info", source: "admin", created_at: new Date(Date.now() - 10800000).toISOString() },
  { id: "5", action: "Verba de campanha acima do limite previsto", entity_type: "financial", severity: "warning", source: "system", created_at: new Date(Date.now() - 14400000).toISOString() },
];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground"><Shield className="w-10 h-10 mx-auto mb-3 opacity-40" /><p className="text-sm">Nenhum registro de auditoria</p></div>
        ) : (
          filtered.map((log) => (
